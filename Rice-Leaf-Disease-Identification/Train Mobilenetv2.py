...
CLASS_NAMES = ['Bacterial_leaf_bright', 'Brown Spot', 'Healthy', 'Leaf Blast']
NUM_CLASSES = len(CLASS_NAMES)

# =========================================================
# 🧠 TẠO MÔ HÌNH MOBILENETV2
# =========================================================
def create_mobilenet_model(img_size=IMG_SIZE, num_classes=NUM_CLASSES):
...
def train_model():
    ...
    print('🏋️‍♂️ Bắt đầu huấn luyện MobileNetV2...')
    history = model.fit(
        train_gen,
        epochs=EPOCHS,
...
# =========================================================
def main():
    print('=== TRAINING MOBILENETV2 FOR RICE LEAF DISEASE DETECTION ===')
    ...
    try:
        ...
        if ans == 'y':
            ...
            model.fit(
                ...
                epochs=10,
                validation_data=val_gen,
                callbacks=[ModelCheckpoint('models/mobilenetv2_finetuned_model.h5', monitor='val_accuracy', save_best_only=True)],
                verbose=1
            )
            evaluate_model(model, test_gen)
